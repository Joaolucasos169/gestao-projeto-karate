...
@jwt_required()
def list_alunos():
    """Lista todos os alunos ativos, com opção de busca."""
    try:
...
def create_aluno():
    ...
    required_fields = ['nome', 'cpf', 'data_nascimento', 'grau_atual', 'sexo']
    ...
    if telefone_para_validar:
        ...
        if telefone_limpo is None:
            return jsonify({"message": "Telefone inválido. Deve conter 10 ou 11 dígitos."}), 400
    ...
    try:
        ...
        new_aluno = AlunoModel(
            ...
            sexo=data.get('sexo'),  # ✅ Campo adicionado
...
def update_aluno(aluno_id):
    ...
    if 'cpf' in data:
        ...
        if cpf_limpo_novo != aluno.cpf:
            existing_cpf = AlunoModel.query.filter(
                AlunoModel.cpf == cpf_limpo_novo,
    ...
    try:
        ...
        aluno.cpf = cpf_limpo
        aluno.telefone = telefone_limpo
        aluno.nome_pais = data.get('nome_pais', aluno.nome_pais)
        aluno.endereco = data.get('endereco', aluno.endereco)
        aluno.grau_atual = data.get('grau_atual', aluno.grau_atual)
        aluno.ativo = bool(data.get('ativo', aluno.ativo))
        aluno.sexo = data.get('sexo', aluno.sexo)  # ✅ Campo adicionado

        # Datas
        if data.get('data_nascimento'):
            aluno.data_nascimento = datetime.strptime(data['data_nascimento'], '%Y-%m-%d').date()
        if data.get('data_ultima_graduacao') is not None:
            ult_grad_str = data['data_ultima_graduacao']
            aluno.data_ultima_graduacao = (
                datetime.strptime(ult_grad_str, '%Y-%m-%d').date()
                if ult_grad_str else None
            )
...
def delete_aluno(aluno_id):
    """Marca o aluno como inativo (soft delete)."""
    aluno = AlunoModel.query.get_or_404(aluno_id)
    try:
        aluno.ativo = False
        db.session.commit()
